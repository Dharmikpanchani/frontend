...
export const planService = {
  getAll: (
    ...
    if (filters.developerName)
      url += `&developerName=${encodeURIComponent(filters.developerName)}`;
    if (filters.developerPhoneNumber)
      url += `&developerPhoneNumber=${encodeURIComponent(filters.developerPhoneNumber)}`;
    if (filters.planName)
      url += `&planName=${encodeURIComponent(filters.planName)}`;
    if (filters.billingCycle)
      url += `&billingCycle=${encodeURIComponent(filters.billingCycle)}`;
    if (filters.priceMin !== undefined && filters.priceMin !== "")
      url += `&priceMin=${encodeURIComponent(filters.priceMin)}`;
    if (filters.priceMax !== undefined && filters.priceMax !== "")
      url += `&priceMax=${encodeURIComponent(filters.priceMax)}`;
