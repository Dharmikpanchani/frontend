...
export default function AddEditAdminUser() {
  ...
  const { selectedAdminUser, loading, schoolPagination } = useSelector(
  ...
  const getRelativePlanExpiry = (
    ...
    const expiry =
      expiryTimestamp < 10000000000
        ? moment.unix(expiryTimestamp)
        : moment(expiryTimestamp);
    ...
    dispatch(
      getAdminUserById({
        ...
        params: {
          ...
          establishedYear:
            filters?.establishedYear ?? filterValues.establishedYear,
        },
      }) as any,
    );
  };
  ...
  const filterFields: any[] = [
    ...
    },
    {
      ...
      placeholder: "Enter School Url name",
    },
    {
      type: "inputSelect",
      name: "panNumber",
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      if (addEditAdminUser.fulfilled.match(resultAction)) {
        ...
        if (!id) {
          navigate("/admin-user/otp", {
            state: { type: "registration", email: values.email },
          });
        } else {
          navigate("/admin-list");
        }
  ...
  return (
    <Box className="admin-dashboard-content">
      ...
      {isView && isSuperDeveloper && selectedAdminUser?.schools?.length > 0 && (
        <Box sx={{ mt: 5 }}>
          <Box
            className="admin-user-list-flex admin-page-title-main"
            sx={{ mb: 2 }}
          >
            <Typography
                ...
                <Table aria-label="simple table" className="table">
                  <TableHead className="table-head">
                    <TableRow className="table-row">
                      <TableCell className="table-th" width="20%">
                        SCHOOL DETAILS
                      </TableCell>
                      <TableCell className="table-th" width="18%">
                        LOCATION
                      </TableCell>
                      <TableCell className="table-th" width="12%">
                        STATUS
                      </TableCell>
                      <TableCell className="table-th" width="15%">
                        ACADEMIC INFO
                      </TableCell>
                      <TableCell className="table-th" width="15%">
                        PLAN INFO
                      </TableCell>
                      <TableCell className="table-th" width="10%">
                        TAX INFO
                      </TableCell>
                      <TableCell className="table-th" width="10%">
                        JOINED
                            ...
                            >
                              ...
                              <Box
                                sx={{
                                  ...
                                  py: 0.2,
                                  borderRadius: "20px",
                                  backgroundColor: data?.isActivePlan
                                    ? "rgba(76, 175, 80, 0.1)"
                                    : "rgba(244, 67, 54, 0.1)",
                                  color: data?.isActivePlan
                                    ? "#4caf50"
                                    : "#f44336",
                                  width: "fit-content",
                                }}
                          ...
                          <TableCell className="table-td">
                            ...
                            {data?.registrationNumber && (
                              <Typography
                                sx={{ fontSize: "11px", color: "#9ca3af" }}
                              >
                                Reg: {data?.registrationNumber || "NA"}
                              </Typography>
                            )}
                          </TableCell>
                          <TableCell className="table-td">
