...
export const passwordValidation = (fieldName: string, required = true) => {
  let schema = Yup.string()
    ...
    .matches(/\d/, `${fieldName} must include at least one number`)
    .matches(/[a-z]/, `${fieldName} must include at least one lowercase letter`)
    .matches(/[A-Z]/, `${fieldName} must include at least one uppercase letter`)
    .matches(
      /[^\w\s]/,
...
export const roleNameValidation = (required = true) => {
  ...
  return required ? schema.required("Role name is required") : schema;
};
...
export const phoneNumberValidation = (required = true) => {
  ...
  return required ? schema.required("Phone number is required") : schema;
};
...
export const schoolCodeValidation = (required = true) => {
  let schema = Yup.string()
    .test(
      "no-whitespace",
      "Please enter a valid school url name",
      (_value, context) =>
        typeof context.originalValue !== "string" ||
        context.originalValue.trim() === context.originalValue,
    )
...
export const schoolNameValidation = (required = true) => {
  ...
  return required ? schema.required("School name is required") : schema;
};
...
export const genericStringValidation = (
  fieldName: string,
  min = 1,
  max = 200,
  required = true,
...
export const dateValidation = (
  ...
  return required ? schema.required(message) : schema;
};
...
export const imageValidation = (
  ...
  const schema = Yup.mixed()
    ...
    .test(
      "fileSize",
      `${fieldName} size must be less than 20MB`,
      (value: any) => {
...
export const fileValidation = (
  ...
  const schema = Yup.mixed()
    ...
    .test(
      "fileType",
      `Invalid file type. Only ${allowedFormats.map((f) => f.split("/")[1].toUpperCase()).join(", ")} are allowed`,
      (value: any) => {
        if (!value || typeof value === "string") return true;
...
export const changePasswordValidationSchema = Yup.object({
  ...
  newPassword: passwordValidation("New password", true).notOneOf(
    [Yup.ref("oldPassword")],
...
export const planValidationSchema = Yup.object().shape({
  ...
  monPrice: Yup.number()
    ...
    .when(["billingCycle", "planName"], {
      is: (billingCycle: string, planName: string) =>
        billingCycle === "6month" &&
        (!planName || planName.trim().toLowerCase() !== "free"),
  ...
  yerPrice: Yup.number()
    ...
    .test("min-yearly-price", "Minimum price not met", function (value) {
      ...
      if ((value || 0) < minPrice) {
        return this.createError({
          message: `Yearly price must be at least ₹ ${minPrice}`,
        });
      }
      return true;
    })
