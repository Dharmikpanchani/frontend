...
const END_WITH_DOMAIN = process.env.NEXT_PUBLIC_END_WITH_DOMAIN || "localhost:3000";
const SUB_DOMAIN_URL = process.env.NEXT_PUBLIC_SUB_DOMAIN_URL || "http://localhost:5000/api/v1";

const getBaseURL = () => {
  if (typeof window === "undefined") return BASE_URL;
  const host = window.location.hostname;
  if (host.endsWith(END_WITH_DOMAIN)) {
